...
def get_scatter_chart(entered_site,slider):
    if entered_site == 'ALL':
        ...
        new_df=spacex_df[(spacex_df['Launch Site']==entered_site) & (spacex_df['Payload Mass (kg)'] >= slider[0]) & (spacex_df['Payload Mass (kg)'] <= slider[1])]
        fig = px.scatter(data_frame=new_df,x='Payload Mass (kg)',y='class',color='Booster Version Category')
        return fig
# Run the app
